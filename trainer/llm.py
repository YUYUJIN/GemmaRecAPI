...
class SpeedMonitoringCallback(TrainerCallback):
    def __init__(self, threshold, check_interval):
    ...
    def on_step_begin(self, args, state, control, **kwargs):
        if self.steps_taken % self.check_interval == 0:
    ...
    def on_step_end(self, args, state, control, **kwargs):
        if self.steps_taken % self.check_interval == 0:
            ...
            avg_time_per_step = elapsed_time / self.check_interval
            
            if avg_time_per_step > self.threshold:
                print(f"Step {state.global_step}: Average time per step {avg_time_per_step:.2f}s exceeds threshold {self.threshold}s. Clearing GPU memory.")
                torch.cuda.empty_cache()
                torch.cuda.ipc_collect()

def gemma_collate_fn_w_truncation(llm_max_length, eval=False):
    def gemma_collate_fn(batch):
        ...
        for i in range(len(batch)):
            ...
            if eval: assert input_ids[-1] == 108
            else:
                assert input_ids[-3] == 108 and input_ids[-1] == 1
...
class LLMTrainer(Trainer):
    def __init__(
        ...
        super().__init__(
            model=model,
            args=hf_args,
            callbacks=[EarlyStoppingCallback(args.lora_early_stopping_patience),MemoryCleanupCallback(),SpeedMonitoringCallback(30,5)],
        ...
        self.train_loader.collate_fn = gemma_collate_fn_w_truncation(self.llm_max_text_len, eval=False)
        self.val_loader.collate_fn = gemma_collate_fn_w_truncation(self.llm_max_text_len, eval=True)
        self.test_loader.collate_fn = gemma_collate_fn_w_truncation(self.llm_max_text_len, eval=True)
