...
def seq_to_token_ids(args, seq, candidates, label, info, comment_dict, item_dict, tokenizer, prompter, eval=False):
    def truncate_text(text):
        text_ = tokenizer.tokenize(text)[:args.llm_max_title_len]
        text = tokenizer.convert_tokens_to_string(text_)
        return text
    try:
        seq_t = ' \n '.join(['(' + str(idx + 1) + ') ' + truncate_text(comment_dict[item]) 
                        for idx, item in enumerate(seq)])
    except:
        print(info,seq,comment_dict)
    can_t = ' \n '.join(['(' + chr(ord('A') + idx) + ') ' + truncate_text(item_dict[item])
    ...
    data_point['input'] = args.llm_input_template.format(truncate_text(info), seq_t, can_t)
...
class LLMDataloader():
    def __init__(self, args, seq_dataset, retrieved_path):
        self.args = args
        self.rng = np.random
        ...
        self.info_dict = seq_dataset['info']
        self.comment_dict = seq_dataset['comment']
        self.item_dict = seq_dataset['item']
        ...
        self.llm_retrieved_path = retrieved_path
        print('Loading retrieved file from {}'.format(self.llm_retrieved_path))
        retrieved_file = pickle.load(open(os.path.join(self.llm_retrieved_path,
                                                       'retrieved.pkl'), 'rb'))
    ...
    def _get_train_dataset(self):
        dataset = LLMTrainDataset(self.args, self.train, self.max_len, self.rng,
                                  self.info_dict, self.comment_dict, self.item_dict, self.tokenizer, self.prompter)
        return dataset
    ...
    def _get_eval_dataset(self, mode):
        if mode == 'val':
            dataset = LLMValidDataset(self.args, self.train, self.val, self.max_len, self.rng, \
                                      self.info_dict, self.comment_dict, self.item_dict, self.tokenizer, self.prompter, self.val_users, \
                                      self.val_candidates)
        elif mode == 'test':
            dataset = LLMTestDataset(self.args, self.train, self.val, self.test, self.max_len, \
                                     self.rng, self.info_dict, self.comment_dict, self.item_dict, self.tokenizer, self.prompter, self.test_users, \
                                     self.test_candidates)
        return dataset


class LLMTrainDataset(data_utils.Dataset):
    def __init__(self, args, u2seq, max_len, rng, info_dict, comment_dict, item_dict, tokenizer, prompter):
        self.args = args
        ...
        for u in sorted(u2seq.keys()):
            ...
            for i in range(2, len(seq)+1):
                self.all_seqs += [[u,seq[:i]]]

    def __len__(self):
        return len(self.all_seqs)

    def __getitem__(self, index):
        tokens = self.all_seqs[index][1]
        info=self.info_dict[self.all_seqs[index][0]]
        comment=self.comment_dict[self.all_seqs[index][0]]
        answer = tokens[-1]
        ...
        return seq_to_token_ids(self.args, seq, candidates, answer, info, comment, \
                                self.item_dict, self.tokenizer, self.prompter, eval=False)


class LLMValidDataset(data_utils.Dataset):
    def __init__(self, args, u2seq, u2answer, max_len, rng, info_dict, comment_dict, item_dict, tokenizer, prompter, val_users, val_candidates):
        self.args = args
        self.u2seq = u2seq
    ...
    def __getitem__(self, index):
        ...
        info=self.info_dict[user]
        comment=self.comment_dict[user]
        seq = self.u2seq[user]
        ...
        return seq_to_token_ids(self.args, seq, candidates, answer, info, comment, self.item_dict, self.tokenizer, self.prompter, eval=True)


class LLMTestDataset(data_utils.Dataset):
    def __init__(self, args, u2seq, u2val, u2answer, max_len, rng, info_dict, comment_dict, item_dict, tokenizer, prompter, test_users, test_candidates):
        self.args = args
        self.u2seq = u2seq
        self.u2val = u2val
        self.u2answer = u2answer
